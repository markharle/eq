// ============================================================================
// TREND TEXT RENDERER
// Renders the currentTrend field from JSON data into the trendText markup
// ============================================================================
...
function renderTrendText() {
  ...
  fetch(CONFIG.JSON_URL)
    ...
    .then(data => {
      ...
      // Step 4: Extract the currentTrend value
      const trendValue = entityData.currentTrend;

      // Step 5: Render the value into the DOM
      document.getElementById("currentTrend").textContent = trendValue;

      console.log(`Successfully rendered trend text for ${CONFIG.ENTITY}: ${trendValue}`);
    })
    .catch(error => {
      console.error("Error fetching or rendering trend text:", error);
      document.getElementById("currentTrend").textContent = "Error loading data";
    });
}
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", renderTrendText);
} else {

...
(function () {
   ...
   *
   *   markerStyle       "default" | "pill" | "custom"  (default: "default")
   *                     "custom" uses a configurable SVG icon defined by:
   *                       customMarkerURL    - absolute URL to the SVG file
   *                       customMarkerHeight - icon height (e.g., "25" or "25px")
   *                       customMarkerWidth  - icon width  (e.g., "25" or "25px")
   *                     Falls back to "default" if any of the three are missing.
  ...
  function initializeMap(targetDiv, listings, popupTemplate, mapConfig) {
    ...
    listings.forEach(function (listing) {
      ...
      // Build marker — default Leaflet pin, custom pill, or custom SVG icon
      var markerOptions = {};

      if (markerStyle === "pill") {
        markerOptions.icon = buildMarkerIcon(listing);
      } else if (markerStyle === "custom") {
        var customIcon = buildCustomMarkerIcon(cfg);
        if (customIcon) {
          markerOptions.icon = customIcon;
        }
        // If buildCustomMarkerIcon returns null, Leaflet default is used
      }
  ...
  /**
   * Builds a custom SVG Leaflet icon using the three mapConfig keys:
   *   customMarkerURL    - absolute URL to the SVG file on S3
   *   customMarkerHeight - icon height ("25" or "25px")
   *   customMarkerWidth  - icon width  ("25" or "25px")
   *
   * Returns null (triggering Leaflet default) if any key is missing,
   * empty, or produces an invalid dimension value.
   *
   * The icon is anchored at its horizontal center and bottom edge so
   * the pin tip lands precisely on the coordinate. The popup opens
   * 4px above the icon top.
   *
   * Further visual customisation (color via CSS filter, etc.) should
   * be applied through the map's custom CSS file using the default
   * Leaflet .leaflet-marker-icon selector.
   *
   * @param  {object}     cfg  - resolved mapConfig object
   * @returns {L.Icon|null}
   */
  function buildCustomMarkerIcon(cfg) {
    var url    = cfg.customMarkerURL;
    var height = cfg.customMarkerHeight;
    var width  = cfg.customMarkerWidth;

    if (!url || !height || !width) {
      console.warn(
        "[ListingsMap] markerStyle 'custom' requires customMarkerURL, " +
        "customMarkerHeight, and customMarkerWidth in mapConfig. " +
        "Falling back to default Leaflet pin."
      );
      return null;
    }

    // Accept "25" or "25px" — parseInt ignores the unit suffix
    var h = parseInt(height, 10);
    var w = parseInt(width,  10);

    if (isNaN(h) || isNaN(w) || h <= 0 || w <= 0) {
      console.warn(
        "[ListingsMap] Invalid customMarkerHeight or customMarkerWidth values. " +
        "Falling back to default Leaflet pin."
      );
      return null;
    }
    ...
    return L.icon({
      iconUrl:    url,
      iconSize:   [w, h],
      iconAnchor: [Math.round(w / 2), h],   // bottom-center anchors to coordinate
      popupAnchor:[0, -(h + 4)]              // popup opens 4px above icon top
    });
  }
  ...
  function extractPopupContent(templateHtml) {
    ...
    styleEls.forEach(function (styleEl) {
      if (!document.querySelector("style[data-map-popup-styles]")) {
        ...
        liveStyle.setAttribute("data-map-popup-styles", "true");
